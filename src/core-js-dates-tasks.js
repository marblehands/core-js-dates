...
function getDayName(date) {
  ...
  const indexOfDay = newDate.getUTCDay();
  switch (indexOfDay) {
    case 0:
      return 'Sunday';
    case 1:
...
function getCountDaysInMonth(month, year) {
  ...
  const secondDate = new Date(nextYear, nextMonth - 1);
  const days = Math.round((secondDate - firstDate) / millisecondsPerDay);
  return days;
}
...
function formatDate(date) {
  ...
  const dateNum = newDate.getUTCDate();
  ...
  return `${month + 1}/${dateNum}/${year}, ${hour}:${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')} ${periodOfDay}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const firstDay = newDate.getUTCDay();
...
function getWeekNumberByDate(date) {
  const shift = (year) => {
    ...
    return (
      startDate.getTime() -
      (dayStart === 0 ? 7 : dayStart) * 24 * 60 * 60 * 1000
    );
  };

  const year = date.getUTCFullYear();
  const weekStart = shift(year);
  const targetTime = date.getTime();

  const milisec = 7 * 24 * 60 * 60 * 1000;
  const result = Math.ceil((targetTime - weekStart) / milisec);
